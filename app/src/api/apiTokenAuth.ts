...
import { isSignedStoreFaviconRequest } from "../services/storeFaviconUrlService";
 ...
 * 外形監視と仕様確認のためにトークン無しで通す。店舗ファビコンの GET は
 * ブラウザの <img> がヘッダーを付けられないため、URL の署名で通す。
 */
...
export const apiTokenAuth: MiddlewareHandler<ApiBindings> = async (c, next) => {
    if (
        ...
        isSignedStoreFaviconRequest(c.env, c.req.raw)
    ) {
        return next();
    }
