...
receiptsApp.openAPIRegistry.registerPath({
    ...
    responses: {
        200: {
            description:
                "The receipt after the apply, together with the purchase id and the per-line outcome. replayed marks a line whose stock was already applied by an earlier attempt with the same idempotencyKey, so the same body is returned for a retry as for the first call. priceRecorded is false for a line with no price, for a line whose stock was replayed, for a line whose explicit contentUnit cannot be converted to the item's base unit, and for a line whose item counts stock in a unit that does not match its own dimension (a mass or volume item whose baseUnit is not one of g, kg, mL, L, compared without regard to letter case); otherwise a count item records contentAmount 1 with setCount equal to the quantity, and a mass or volume item records the quantity as the content of a single set. aliasRegistered is false when the receipt wording already belongs to another item. The returned receipt carries the same outcome on its lines: each applied line gets applied with the quantity, base unit, price and expiry date that were recorded, and the receipt gets appliedTotalPrice. A replayed line keeps the values written by the attempt that actually moved its stock, so a retry that sends different values does not overwrite them.",
