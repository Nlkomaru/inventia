...
export const registerInventoryWriteTools = (
    ...
    server.registerTool(
        "adjust_inventory_stock",
        {
            ...
            description:
                "Applies a signed stock delta to the item's expiry lots: lot quantities change, an immutable stock movement with its per-lot allocations is recorded, and the item's total quantity is recomputed from the lots. delta must not be 0 and reason states why the stock moved. A positive delta is added to the lot with the given expiryDate and creates that lot when it does not exist yet; omitting expiryDate or sending null targets the lot without an expiry date. A negative delta is taken from the lot named by lotId or expiryDate, and omitting both consumes lots in FEFO order (earliest expiry first, the lot without an expiry date last). lotId and expiryDate cannot be sent together. note is free-text describing what the stock was used for, such as cooking a meal, and complements reason, which is a fixed category. externalProviderId is the id of an external provider from list_external_providers and records the app the stock went to; sending an id that does not exist is rejected with EXTERNAL_PROVIDER_NOT_FOUND. externalId is that app's own identifier for the record: Inventia stores and displays it but never interprets or resolves it, and it can only be sent together with externalProviderId. The recorded movement returns note, externalId, and externalProvider resolved to the provider's current name, faviconUrl, and url. idempotencyKey is required because this operation is not repeatable: retrying with the same key and the same request returns the already stored operation with replayed true instead of applying the delta twice, while the same key with a different request - including a different note, externalProviderId, or externalId - is rejected as a conflict rather than overwriting the stored operation.",
