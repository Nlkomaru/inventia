	...
	columnHelper.accessor("name", {
		...
		cell: ({ getValue, row }) => (
			<div className="min-w-56">
				...
				<p className="mt-1 text-xs text-slate-500">{row.original.category}</p>
			</div>
		),
	}),
	...
	return (
		<div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_18px_50px_-36px_rgba(15,23,42,0.45)]">
			<div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
				<div>
					<h2 className="text-sm font-bold text-slate-950">在庫一覧</h2>
					<p className="mt-1 text-xs text-slate-500">{data.length} 件の品目</p>
				</div>
