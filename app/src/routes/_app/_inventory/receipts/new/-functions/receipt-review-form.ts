...
export interface ReceiptReviewRow {
     ...
     * そのまま足さない）。
     *
     * 単位を換算できない組み合わせでは「数量を品目の単位に直して指定してください」
     * と返るが、その指示どおり解析値と同じ数を打ち直すと値比較では未編集になり、
     * 数量が省略されて同じエラーが永久に返る。反映は途中で throw するため先行する
     * 行は在庫が動いたままになり、レシートは部分反映のまま二度と完了できなくなる。
     * 編集したという事実そのものを持たせて脱出口を残す。
     */
...
export const buildApplyLine = (
    ...
    const common = {
        ...
        // 単位を換算できない組み合わせでサーバーが返す「数量を品目の単位に直して
        // 指定してください」に従って同じ数を打ち直しても未編集と見なされ、
        // 数量が省略されて同じエラーが永久に返るため（ReceiptReviewRow の
